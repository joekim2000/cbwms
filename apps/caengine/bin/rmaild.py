...
class Daemon:
	...
	def run(self):
		...
		self.mail = rmail.Mail(daemonName)

		self.isStop = False
		while (not self.isStop):
			for user in self.mail.cfg.Users:
				if self.mail.Open(user[0], user[1]):
					if self.mail.Get():
						self.mail.RemoveOldList(self.mail.cfg.RemoveBeforeDays)
				if (self.isStop):
					break
			time.sleep(1)
...
def Test(userno, arg=''):
	print 'Test'
	mail = rmail.Mail(daemonName, True)
	mail.Open(mail.cfg.Users[userno][0], mail.cfg.Users[userno][1])
	if mail.Get(arg):
		mail.RemoveOldList(mail.cfg.RemoveBeforeDays)
	mail.Close()
